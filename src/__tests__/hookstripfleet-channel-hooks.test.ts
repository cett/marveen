...
describe('HOOKSTRIPFLEET913: the four channel hooks survive the #1307 strip', () => {
  ...
  describe('mutant control: the template is the only source', () => {
    ...
    it('the shipped template carries all four; a mutant with them dropped carries none', () => {
      ...
      // provenance-gate.py is not ported on this fork (missing script,
      // #885 B3 review); staleness-guard.py is the fork's own equivalent
      // pre-existing, unrelated governance hook.
      expect(mutant.has('staleness-guard.py'), 'mutant keeps governance hooks').toBe(true)
    })
