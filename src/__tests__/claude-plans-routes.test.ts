...
// (mirrors approvals-notify.test.ts's fake req/res harness). STORE_DIR points
// at a real temp dir so the CRUD round-trips through the actual atomic-write
// path. hardRestartMarveenChannels/restartAgentProcess are mocked -- this
// test must NEVER touch a real tmux session or process.
...
describe('POST /api/claude-plans/rotate (PR2c)', () => {
  ...
  it('returns 500 and does not report ok when the main-agent restart fails', async () => {
    ...
    expect(out.body.hint).toContain('launchctl boom')
  })
