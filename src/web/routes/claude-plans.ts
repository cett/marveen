...
// on this fork). PR2b added POST/PUT/DELETE on store/claude-plans.json plus a
// read-only GET .../state. PR2c adds the write side of the state side-car
// (POST .../rotate) -- the one code path both a (not-yet-built) manual
...
export async function tryHandleClaudePlans(ctx: RouteContext): Promise<boolean> {
  ...
  if (path === '/api/claude-plans/rotate' && method === 'POST') {
    ...
    if (!targetPlanId) {
      json(res, { error: 'required', hint: 'targetPlanId is required' }, 400)
      return true
    }

    if (!isRotationEnabled()) {
      json(res, { error: 'disabled', hint: 'Rotation is disabled (CLAUDE_ROTATION_ENABLED=0)' }, 409)
      return true
    }
    ...
    if (!target) {
      json(res, { error: 'not_found', hint: `Unknown target plan id: ${targetPlanId}` }, 400)
      return true
    }
    ...
    if (!target.channelsAllowed) {
      json(res, {
        error: 'invalid_value',
        hint: `Plan ${targetPlanId} does not allow channels use (channelsAllowed=false)`,
      }, 400)
      return true
    }
    ...
    if (agentId === MAIN_AGENT_ID) {
      ...
      if (!isMainAgentIsolated() || plans.length < 2) {
        json(res, {
          ...
          hint: 'Rotation not applicable: main agent needs MAIN_AGENT_ISOLATED_CONFIG=1 and at least 2 registered plans',
        }, 409)
        return true
      }
      ...
      if (!result.ok) {
        json(res, { error: 'internal_error', hint: result.error || 'Main agent restart failed' }, 500)
        return true
      }
    ...
    if (!existsSync(agentDir(agentId))) {
      json(res, { error: 'not_found', hint: `Agent not found: ${agentId}` }, 404)
      return true
    }
