...
- Role/permission matrix on the dashboard's Felhasznalok > Felhasznalok tab: a read-only table showing every role (admin/agent/read_only/viewer) against every permission from `src/web/rbac.ts`, grouped by category (memory, tasks, agents & communication, approvals, federation, administration). The frontend keeps a manually-synced mirror of `rbac.ts`'s permission set (a live endpoint was judged unnecessary indirection for config that changes at the same pace as its own edits) -- a new test asserts every cell in that mirror matches `hasPermission()` from the real `rbac.ts`, so a drift between the two fails CI. Both `hu.js` and `en.js` gained the matching category/permission i18n keys
