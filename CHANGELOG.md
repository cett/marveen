...
- CI: `pip-audit` step scanning the two pinned Python requirements files (`scripts/gdocs-requirements.txt`, `scripts/watchdog-requirements.txt`) for dependency CVEs -- the Python-side equivalent of the existing `npm audit` step (blocking, same as `npm audit --audit-level=high`). Plus a Semgrep JS/TS static security scan (`p/security-audit` + `p/secrets` + `p/javascript` + `p/typescript` rulesets over `src/`, `web/`, `scripts/`) that catches in-repo patterns `npm audit`'s dependency-only scope can't (injection shapes, hardcoded-secret patterns, misconfigured crypto/CORS). Starts report-only (`continue-on-error: true`) until the first findings are triaged -- 25 exist today across the tree, mostly one repeated low-signal pattern (`dynamic-urllib-use-detected` in the hook scripts) plus a few that look worth a look (a CORS-misconfiguration pattern in `src/web.ts`, `createDecipheriv` missing an explicit GCM auth-tag length in `src/web/fleet-transfer.ts` and `src/web/vault.ts`, an `insertAdjacentHTML` call in `web/modules/memory-graph.js`) -- flip to blocking once that triage happens. JSON results also uploaded as a build artifact (`semgrep-report`) for deeper review beyond the build log's summary (#816, per #814's Suranyi-cikk 4-gate assessment)
