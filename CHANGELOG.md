...
- `scripts/channels.sh`'s vault-inject-http-mcp.sh inject/restore cycle now also covers the isolated main-agent config at `.channels-config/.claude.json`, not just the default `~/.claude.json` -- when the channel bot runs in isolated main-agent-config mode (`MAIN_AGENT_ISOLATED_CONFIG=1`), its actual `CLAUDE_CONFIG_DIR` resolves to `.channels-config`, which carries its own, separate `mcpServers.*.headers` block that the provisioner (additive-only) never propagated a vault: template into -- left uncovered, that config's HTTP MCP headers (e.g. UptimeRobot's) sat as plaintext at rest. Both inject calls are independent and idempotent (own `.vault-template` guard each), so the added call is a safe no-op on installs that don't use isolated mode (#761 [3.1], structural part -- the actual header value edit + main-session restart is a separate, deliberately manual step)
