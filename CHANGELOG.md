...
- Memory search detail modal (`web/modules/memories.js`, kanban 87076b19): clicking a memory result card now opens a read-only modal (`#memDetailOverlay`) instead of the inline expand toggle -- tier-colored side strip, agent/date/salience badges, markdown-rendered content, and keyword tags with the matched ones highlighted. The active search query is highlighted in the rendered content via a DOM `TreeWalker` pass over `renderMarkdown()`'s output (never over the raw markdown source, since `renderMarkdown` HTML-escapes its input and would otherwise swallow an injected `<mark>` tag). An Edit button routes into the existing edit modal. Workspace-doc result cards are unaffected -- they never had a click handler and still don't. No API changes.
