...
- Backend coverage step 2 of #751 (85% target, gradual): `src/web/routes/docs.ts` (`GET /api/docs`, `GET /api/docs/:name`) was at 0% -- fully covered now (listing/title-extraction/sort order, path-traversal rejection, 404/400 paths, unmatched-route passthrough). `vitest.config.ts`'s coverage floor ratcheted up to match (statements 56->57, branches 55->56, lines 57->58), ~2 points below the actual measured numbers to absorb normal jitter
