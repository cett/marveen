...
- a Súgó/Help chapter index (`#help`, reachable from a new sidebar nav link) listing every chapter of both the user guide and the installation/operations guide, language-aware, each entry linking straight into the existing help viewer (`#help/<doc-path>`). Previously the installation/operations guide's chapters were served and viewable but had no UI entry point of their own -- only reachable via a hand-typed hash. Frontend-only (`web/modules/help.js`, `web/modules/app-core.js`, `web/index.html`, `web/lang/hu.js`/`en.js`): no new backend route, the existing `GET /api/docs/<path>` viewer is unchanged
- **[API]** the dashboard now serves the user-guide docs over HTTP instead of pointing at GitHub: a new `GET /api/docs/<path>` route (revived from a `docs.ts` route retired in an earlier docs cleanup, allowlisted to `user-guide/{hu,en}/<slug>.md` and `fork-guide/{hu,en}/<slug>.md`, path-traversal-guarded by regex plus a resolved-path containment check) sits behind the same auth gate as every other user-facing `/api/*` route (RBAC: `memories:read`, the same tier as `/api/workspace`/`/api/recall`), so a logged-in B2B tenant user can read the guide without repo access. Every dashboard view's "? Súgó"/"? Help" link now opens the dashboard's own help viewer (`#help/<doc-path>`, a new lazy-loaded `web/modules/help.js` page rendered with the existing shared `renderMarkdown()`) in a new tab instead of a GitHub blob URL
- Settings screen (#953): the "Beallitasok" page's tab layout regrouped from one tab per registry module (15 tabs) into 9 fixed section tabs (Rendszer, Csatornak & Biztonsag, Agensek & Heartbeat, Kanban, Memoria & Munkadokumentumok, Fleet monitor, Adatmegorzes & Megfigyelhetoseg, Autonomia, Budgetek & Claude csomagok), several with labelled subsections (e.g. Kanban splits into WIP limitek / WIP badge szinek / Auto-archivalas / Aging jelzok / Megjelenites / Otletlada). The former standalone Security tab folded into Csatornak (auth card now sits at the top of that panel); the former standalone CostOps and Claude Plans tabs merged into one Budgetek & Claude csomagok panel with two subsections. Frontend-only (`web/modules/settings.js`'s `loadSettings()` tab-generation logic, `web/lang/hu.js`/`en.js`): no registry key, module id, or the read/write API contract changed -- `src/config-registry.ts` and `src/web/routes/settings.ts` are untouched. A new `scripts/settings-verify.sh` dumps every registry setting's live value (sorted by key) for a before/after diff around the deploy, to prove zero value drift from the regrouping
- **[API]** unified kanban card search: a new `GET /api/kanban/search?q=` endpoint matches across both active and archived cards by running number (`#NNN`), a 4-8 character hex id prefix, or a title/project/assignee substring, ranking active cards ahead of archived ones. The dashboard sidebar gained a global search box (any page, debounced, min. 2 characters) that jumps straight to a matching card -- the active board's detail panel for an active card, or the archived view (pre-filtered and highlighted) for an archived one. The archived view's own search field now uses this same endpoint instead of its previous archived-only text search, which is retired; the archived view's project/date-range filters are unaffected
- install-time seeding of the default tenant's `main_agent_id` and (optionally) its `display_name` on macOS/Linux/Windows-WSL installs, so the Agents screen shows the right tenant-main-agent badge from first boot instead of staying unset until an operator sets it by hand; and a new `system_config` key/value table (schema-only for now, no reads/writes wired up yet) as the foundation for upcoming runtime-configurable system settings
- boot-time `cfg()` (`src/config.ts`) now checks the `system_config` table first, ahead of `/run/secrets/<KEY>`, `config-overrides.json` and `.env`, via a lazy read-only sqlite lookup cached for the process lifetime; `config-overrides.json` continues to be read the same way as before (unchanged for this step)
- `setOverride()` (the `/api/settings` write path) now writes exclusively to `system_config` (`source='db'`) instead of `config-overrides.json`; `getEffectiveSettingValue()` reads `system_config` first, falling back to the (now read-only) `config-overrides.json` and then `.env`, so previously-set values stay readable through the transition. Five new `SETTINGS_REGISTRY` entries: `TELEGRAM_BOT_TOKEN` and `ALLOWED_CHAT_ID` (both `secret: true`; see below for their write path), `CHANNEL_PROVIDER` (`valueSet` covering all five supported providers), `MAIN_AGENT_ID`, and `WEB_PORT`
- `GET /api/settings` now includes `secret: true` rows (`TELEGRAM_BOT_TOKEN`, `ALLOWED_CHAT_ID`) with their value masked as `***` instead of excluding them entirely, so the dashboard can show that they exist and are configured; `POST /api/settings` opens a write path for them gated by a standalone `ctx.role === 'admin'` check (independent of the fleet-wide `RBAC_MODE` shadow/enforce toggle, since shadow mode never blocks via the global gate) -- re-submitting the literal `***` mask is treated as a no-op rather than overwriting the real value. New masked-secret editor UI on the Settings page's Channels tab, admin-only (disabled for non-admin, same "global admin" gate as the rest of the dashboard's admin-only controls). A secret key's real value is also masked in the write's audit trail (`config_change_log` DB rows and the pino log) -- opening the write path is what first made a plaintext secret reach either of those
- `install-macos.sh`'s Ollama startup replaces its fixed 3s `sleep` after `ollama serve &` with a wait-probe (retries `GET /api/version` for up to 30s before giving up), since a fixed delay doesn't guarantee the server is ready before the model pull runs and could hit "connection refused" on a slower machine or a cold start; on timeout it warns and continues (best-effort, as before) instead of failing the install. `install-linux.sh`'s bun install now also appends its `BUN_INSTALL`/`PATH` export to `~/.profile` (created if missing) in addition to `~/.bashrc`/`~/.zshrc`, since a login-but-non-interactive shell (e.g. `su -l`, cron, `systemd --user`) skips most distros' `.bashrc` via its own interactive-only guard, and a minimal image may ship neither `.bashrc` nor `.zshrc` at all -- `.profile` is read by login shells regardless of interactivity, so bun stays on `PATH` for later steps and the next login
- a one-time, idempotent migrator (`migrateConfigOverridesToSystemConfig`, wired into `initDatabase()`) that backfills existing `config-overrides.json` entries into `system_config` with `source='migrated_from_json'`, never overwriting a value already present in the table; plus `getSystemConfig`/`setSystemConfig`/`listSystemConfig` read/write helpers for the table
- `config-overrides.json` deprecation, phase 1 (warn-only, no behaviour change): `cfg()` (`src/config.ts`) now reports which precedence layer resolved each boot-time key (`resolveCfgWithSource`, alongside the existing value-only `resolveCfgPrecedence`), and logs a warning for every key that still resolves from `config-overrides.json` instead of `system_config` -- `"<KEY> resolved from deprecated config-overrides.json; migrate to system_config DB"` -- once the DB and logger are up at boot. Reading `config-overrides.json` is unchanged for now; this only makes it visible in a live system's logs which keys still need the migrator (or a manual `system_config` write) before the file can be retired
- `config-overrides.json` deprecation, phase 2: the file is fully retired. `cfg()`'s boot-time resolution chain is now `system_config` DB > `/run/secrets/<KEY>` > `.env` (the `override` layer, `CFG_OVERRIDE_RESOLVED_KEYS` and its S8A warn-loop in `src/index.ts` are removed, since nothing can ever produce an `'override'` source again); `settings-store.ts`'s `getEffectiveSettingValue()` drops its JSON-cache fallback layer (`db` > `.env` > registry default, `OVERRIDES_PATH`/`getOverrides()`/`reloadOverridesForTest()` removed); `store-watcher.ts` no longer denylists `config-overrides.json` (an event for it is now audited like any other unknown file) and instead denylists the `.deprecated` suffix its retirement rename produces. **Intentional behaviour change**: a key that only ever lived in `config-overrides.json` and was never migrated into `system_config` no longer resolves at all -- it now falls straight through to the `.env`/registry-default layer. `src/web/fleet-transfer.ts`'s identity-takeover block (fleet import) no longer writes `config-overrides.json` alongside its `.env` mirror -- **`.env` (via `updateEnvFile`) is now the only identity-persistence path there**, since every boot-time identity consumer (`OWNER_NAME`, `BOT_NAME`, `BRAND_NAME`, `MAIN_AGENT_ID`) already read `env['KEY']` directly, never `cfg()`, making the JSON write redundant. A new one-time, idempotent, logged migration step (`retireConfigOverridesFile()`, called from `src/index.ts`'s real process boot right after `initDatabase()` -- deliberately NOT from `db/index.ts`'s `initDatabase()` itself, to avoid racing the file against every test file's own `beforeEach`) renames an existing `store/config-overrides.json` to `config-overrides.json.deprecated` as the manual rollback net, once `migrateConfigOverridesToSystemConfig()` has guaranteed every key it held is already a `system_config` row
- test-stability: `db-system-config.test.ts`, `settings-store.test.ts` and `settings-route-db-only-write.test.ts` each get their own per-test `MARVEEN_STORE_DIR` (a fresh `mkdtempSync` directory + `vi.resetModules()` + a dynamic re-import of `db.js`/`settings-store.js`/`web/routes/settings.js`, since `STORE_DIR` is a module-level const resolved once at import time). Previously these three files all wrote/read `config-overrides.json` against the real, shared worktree `store/` directory with no isolation between them, which are separate vitest worker processes -- a flaky cross-file race (reproduced 4 times out of 5 stress runs on the pre-fix commit). No production code changed
- unit test coverage for `store-watcher.ts`'s watch-callback orchestration (system-file/regex/directory denylist filtering, creation-vs-delete-vs-replace classification via `knownFiles`, rename-event dedup, write-actor attribution and its consumption on every event including denylisted ones, and `logStoreFileEvent` failure handling), raising the file from ~37% to a substantially higher statement coverage
- unit test coverage for `telegram-inbox-wake.ts`'s I/O wrapper (`maybeWakeSubAgentsForTelegram`): the opt-in flag early-out, agent-registry failure handling, the main-agent skip, the per-agent inbox state machine (missing/empty/fresh/stuck file), the cheap-gates-before-tmux-I/O ordering (debounce and attempt-budget gates short-circuit before any session probe, and a nonexistent session skips the idle probe), the full wake path, the per-agent attempt-budget exhaustion and its reset on a new inbound backlog, and per-agent error isolation, raising the file from ~39% to 100% statement coverage
- unit test coverage for `graph-mail.ts`'s I/O surface (credentials-file mtime cache, OAuth client-credentials token cache including 60s-pre-expiry refresh and clientId-rotation invalidation, and `listMessages`/`sendMail`/`verifyAccess` including their `top` clamping, folder/unread filtering, recipient trimming, the 202-Accepted-as-success case, and non-ok error paths), raising the file from ~29% to ~98% statement coverage
...
- New `closeDatabase()` (`src/db/connection.ts`) flushes the SQLite WAL and closes the handle cleanly, wired into the existing shutdown sequence in `src/index.ts` (same `SIGTERM`/`SIGINT` handler that already drains the HTTP server and clears timers) so a `systemd`/`launchd` stop or container stop no longer leaves the WAL/journal in a torn state. Best-effort and safe to call twice or before a handle was ever opened.
...
- Optional agent caller-identity signal: a fleet agent may send its own id in an `X-Agent-Id` request header (never itself a source of auth/role/tenant truth -- purely a self-reported hint). `POST /api/memories` now warns (`logger.warn`, never blocks) and returns `owner_mismatch: true` in its still-200 response when the header disagrees with the `agent_id` the write claims to act as; a caller that omits the header (most fleet callers today) never triggers this. Adapted from upstream Szotasz/marveen PR #1357's agent-caller-identification piece -- its auto-generated CLAUDE.md scaffold section (`ensureAgentIdHeaderSection()`) was deliberately NOT ported (flagged high-risk, this fork's CLAUDE.md generation already diverges from upstream's); the header convention is instead documented by hand in the `fleet-dashboard-api` skill and in each agent's own CLAUDE.md.
- New, separate `scripts/hooks/destructive-gate.py` PreToolUse hook (adapted from the same upstream PR #1357, its own self-contained file, kept apart from `post-tool-injection-gate.py` which scans tool OUTPUT rather than input): blocks a short, deliberately narrow list of irreversible operations (`rm`, `mv`, `shred`, `sudo`, `mkfs`, `dd`, `git push`, reads of `.env` or credential directories like `.ssh`/`.aws`/`.gnupg`) regardless of permission mode, since a settings.json deny list is inert under `--dangerously-skip-permissions`. Context-sensitive scanning (ported from upstream's own measured false-positive fix) blanks comment lines, a heredoc body written to a file (but keeps one handed to an interpreter, since that one executes), and quoted prose containing whitespace, before pattern-matching -- upstream measured this cutting its false-positive rate from 28.4% to 4.9% with zero loss of true blocks. Quote-aware segment splitting and wrapper-transparency (`xargs`, `timeout`, `bash -c`, `find -exec`, ...) close the gap where a banned command passed as an argument to a wrapper would otherwise slip through. Not wired into any `settings.json` PreToolUse array by this change -- registering a new, previously-untested-in-production blocking gate fleet-wide is a policy decision for the coordinator/owner, left as a deliberate follow-up rather than a side effect of porting the file (see the fleet-wide activation entry below).
- Tenant-IDOR verification for the `workspace_docs` recall path: `vectorSearchDocs`'s ANN-hit join back to `workspace_docs` was already SQL-level tenant-scoped (`WHERE tenant_id = ?` alongside the `doc_id IN (...)` clause from the vec0 match), matching `searchWorkspaceDocs`'s existing FTS-path scoping and `hybridSearchDocs`'s existing RRF-fusion test -- but, unlike those two, it had no unit test actually exercising the ANN branch (Ollama is unavailable in CI, so `generateEmbedding` always short-circuited to the FTS-only fallback). A new test mocks `generateEmbedding` to force the vector path and proves an ANN hit never crosses a tenant boundary even when two tenants embed identical content. No code change was needed -- tenant IDOR on this path is confirmed closed at the SQL level, not just by a post-filter.
- Opt-in `scripts/hooks/browser-content-notice.py`: a PostToolUse hook that gives browser MCP tools (`mcp__playwright__.*`, `mcp__chrome.*`) and `WebSearch` the same `<untrusted>` framing `wrapUntrustedFetch()` already gives `WebFetch` -- both routes bring back external page text, but only `WebFetch` was wrapped, so a browser/search result read exactly like the agent's own reasoning to a prompt injection. The hook wraps the response in a nonce-tagged `<untrusted>` envelope (shape-preserving: a string stays a string, MCP content blocks stay blocks, an object's free-text leaves are wrapped in place, everything else is left untouched) and, since a built-in tool's replacement is silently dropped by the harness when it doesn't match that tool's own output schema, also injects an `additionalContext` label that cannot be shape-rejected -- so the notice survives even when the envelope itself does not. Every read's full payload is appended to owner-only (`0600`) `store/browser-content.log`; a match against the same Level-A injection-pattern classes `post-tool-injection-gate.py` already scans for additionally mirrors a lightweight, pattern-only record into `hook_audit_log` (`POST /api/hook-audit`, best-effort, never affects the hook's own always-exit-0 verdict) so the dashboard sees it without reading the log file -- a clean page produces no such row, on purpose. Adapted from upstream Szotasz/marveen PR #1426: this fork has no `docs/security-hardening.md` (the explanation lives in the hook's own module docstring instead) and gains the `ledger_lib`/`hook_audit_log` dashboard-visibility integration upstream's file-only version doesn't have. Not registered anywhere by default -- an operator who runs a browser MCP adds the hook to their own `settings.json` PostToolUse array.
- Reference-based scheduled-task delivery for large task bodies (adapted from upstream Szotasz/marveen PR #1396): when a scheduled task's prompt body exceeds `SCHEDULED_TASK_INLINE_MAX_CHARS` (1,500 chars), the runner no longer pastes the whole body through tmux send-keys -- a known source of size-driven corruption on large fires -- and instead snapshots it to an immutable `store/scheduled-runs/<timestamp>-<task>-<rand4>.md` file (new `src/web/scheduled-run-snapshot.ts`, atomic write, 0600) and sends only a short `<scheduled-task ... body-file="..." body-sha256="..." body-chars="...">` reference (`wrapScheduledTaskByReference()` in `src/prompt-safety.ts`) that tells the agent to Read the file in full rather than act on a partial paste. A snapshot-write failure (e.g. full disk) falls back to the existing inline path instead of dropping the task. Retention: 7 days, but always keeps each task's most recent 20 snapshots regardless of age (hourly sweep wired into `src/index.ts`, same cadence as the other startup sweeps); `src/store-watcher.ts`'s file-created audit now excludes the whole `scheduled-runs/` directory by filename-varying prefix (otherwise ~100 files/day would drown real agent-created-file audit rows). A separate size-guard (independent of the delivery-path switch, which is unaffected either way) logs a same-day-deduped `logger.warn` + inter-agent notice to the main agent at `SCHEDULED_TASK_BODY_WARN_CHARS` (20,000 chars) and again, more urgently, at the existing `MAX_SCHEDULED_TASK_PROMPT_LEN` (50,000 chars) -- dashboard-only, no direct Telegram send, matching this file's existing pattern for scheduler notices (catch-up summary, pending-retry, task-timeout). Adapted, not cherry-picked: the fork's `scripts/backup.sh` uses an explicit allowlist rather than upstream's `STORE_SKIP` denylist, so the new snapshot directory needed no backup-script change at all (it was never going to be swept up); `src/pane-state.ts` needed no change either -- its existing `<scheduled-task[\s>]` / `</scheduled-task>` pattern matching is already generic enough to recognize the reference variant's opening/closing tags without modification.
- Context-guard daily-handoff tier (`dailyHandoffEnabled` / `dailyHandoffTime` on the per-agent `ContextGuardConfig`, `PUT`/`GET /api/agents/:name/context-guard`): requests a HANDOFF.md + fresh restart at a fixed local wall-clock time each day, independent of context size, for a predictable session lifecycle instead of one keyed only to whatever tier happens to trip. Ranked below the existing wedge/idle-flush tiers (an urgent or costly session is rescued on those grounds first) and defers instead of firing while the pane is busy. Default off and unset, so an existing `context-guard.json` entry cannot switch it on. This is the daily-handoff piece deferred from the Wave D backport above pending a design review (done: kanban 83baf8a7).
- Claude Plans registry DB mirror (`claude_plans_registry`) and per-agent active-plan lifecycle table (`agent_active_plans`): manual, rotation, and handoff-recovery activation sources; deactivated on blackboard done/stale/clean-stop; swept after `PLAN_STALE_MIN` idle minutes (default 90); cascade-delete on plan removal. `GET /api/blackboard` gains an `activePlan` field; the dashboard blackboard panel shows an inline plan-type chip. `store/claude-plans.json` remains the boot-critical source of truth for pre-server scripts (`scripts/main-agent-isolated-config.mjs`); the DB is a best-effort mirror written alongside every existing file write. Context-guard restart writes a plan-id marker into HANDOFF.md; the next launch recovers it (`source='handoff-recovery'`). Sub-agents only -- the main agent's hardRestartMarveenChannels path is not wired in this revision.
- Upstream backport (Szotasz/marveen, selective cherry-pick): macOS installer offers Discord alongside Telegram with Linux parity; Slack managed-settings write is now atomic and never rebuilds org-policy; fleet hook registrations moved from user-global `~/.claude/settings.json` to the repo-tracked `.claude/settings.json` and `templates/settings.json.template` in portable `$CLAUDE_PROJECT_DIR` form, with a scaffold-level write-refusal for the main agent and hook-stripping for isolated-config-dir clones; four channel hooks (channel-inbox-drain, voice-reply-directive, telegram_progress, skill-usage-capture, channel-image-resize, egress-gate) registered in template before the hook-strip that removes them from sub-agents; watchdog self-locates its fleet root instead of assuming `~/marveen`; config-root watchdog reads from the actual config dir; claude-plans pure rotation logic (PR2a), write API and dashboard UI (PR2b), rotate endpoint + `channels.sh` dynamic configDir + heartbeat wiring (PR2c). Wave D (heartbeat lag-metric inject, context-guard daily-handoff trigger) deferred: the heartbeat-metrics-inject subsystem is not present in this fork and cannot be ported mechanically; the context-guard daily-handoff change conflicts with fork-local context-guard divergence and is deferred to a dedicated PR with a design review.
- Fleet-wide activation of the destructive-command PreToolUse gate (`scripts/hooks/destructive-gate.py`), after owner approval: a new `injectDestructiveGate()`/`ensureDestructiveGate()` pair in `src/web/agent-scaffold-hooks.ts`, structurally identical to the existing egress-gate injector (same dedupe/idempotency, same fail-closed contract), registers a `Bash`-matcher PreToolUse hook in every agent's `settings.json` -- applied on every scaffold write (respawn-safe) and backfilled into existing agents at server startup, mirroring how the egress gate already reaches the fleet. The command is a fail-closed `python3` preamble (`command -v python3 || exit 2`, never a silent pass-through) followed by the gate script. The main agent's copy lives in the repo-tracked `.claude/settings.json`, the same pattern the WebFetch egress gate already uses there. `destructive-gate.py` itself is unchanged by this entry -- only its wiring.
...
- **[API]** `GET /api/memories` gains offset-based pagination for the plain listing (no `q`): the response becomes `{ memories, total, offset, limit }` instead of a bare array, with `total` a real `COUNT(*)` -- previously a listing without a search term was hard-capped at `limit` (default 50, max 200) with no way to see anything older. A new `offset` query param (default 0) pages through it; the underlying `getAgentMemories`/`getMemoriesForChat` (plus new `countAgentMemories`/`countMemoriesForChat` counterparts) push the tier/category filter into SQL ahead of `LIMIT`/`OFFSET` so pages stay consistent-sized. Reuses the shared `web/modules/paginator.js` component from the audit trail (see below) on the Memories dashboard page. Deliberately out of scope: a hybrid/keyword search (`q` set) is a ranked top-K result, not a stable offset-able list, so `offset` has no effect there and the response keeps its existing shape (a plain array, or `{ memories, workspace_docs }` with `include_docs=1`); the rarely-used `tier=import` pseudo-filter (matching `agent_id='import'`, not a real category) also keeps its old, unpaginated behavior.
- **[API]** `GET /api/audit-log` gains offset-based pagination: the response is now `{ entries, total, offset, limit }` instead of `{ entries, total: entries.length }` (the previous "total" was actually just the page size, not a real count), and an `offset` query param (default 0) pages through the merged, multi-source result -- previously the endpoint silently truncated at 200 rows with no way to see anything older. `queryAuditLog()` fetches each active source's own top `offset + limit` rows (there is no single shared index to page a UNION of independently-sorted sources on), merges and re-sorts once, then slices out the requested page; `total` is now a genuine per-source `COUNT(*)` sum, unbounded by limit. A new shared `web/modules/paginator.js` component (a small "◄ n-m / N ►" control, styled via `web/css/components/paginator.css`) renders the page control and wires its Prev/Next buttons without any opinion on where the next page's data comes from, so it can back both server-refetch pagination (used here, on the Teljes audit trail page) and future client-side-slice pagination the same way. The Teljes audit trail page's filters (source checkboxes, agent/query text) reset the page back to offset 0 on change, same as before.
- The `scripts/statusline-ratelimit.sh` script (added for the Overview quota strip) is now actually wired up as the Claude Code `statusLine` command, in both `.claude/settings.json` and `templates/settings.json.template` (the latter with a `{{PROJECT_ROOT}}` placeholder for fresh installs), so it actually produces `store/.claude-rate-limits.json` at runtime instead of only being ready to.
- **[API]** Ported upstream Szotasz/marveen#1229: an admin-only subscription quota strip on the Overview page (5-hour + 7-day window, bar + percentage + reset countdown), read from `store/.claude-rate-limits.json` via a new `scripts/statusline-ratelimit.sh` (a Claude Code statusLine command, copied directly from upstream since it has no fork-side dependency) and a new `src/web/quota.ts`/`readQuotaSnapshot()`. `GET /api/overview` gains a `quota` field inside its existing admin-only response spread (fleet-level data, one shared pool, not per-tenant), so a non-admin caller's response is unchanged; the frontend (`web/modules/overview.js`) hides the strip entirely for a non-admin view rather than upstream's own "still show a quiet placeholder" behavior, since even that would leak the field's existence. A reading older than `QUOTA_MAX_AGE_SEC` (default 6h, same default the existing `limit-monitor.sh` uses) or a window past its own reset time keeps its numbers but loses its color, so a stale reading never looks as trustworthy as a fresh one. CSS re-authored into `web/css/features/overview.css` against the fork's own design tokens (the upstream PR's `web/style.css` target does not exist here).
...
- Backend coverage -> 85% target (#751, gradual, all steps landing on this one feature branch rather than a PR per module): `src/web/routes/background-tasks.ts` (the ad-hoc background-task API: `POST`/`GET /api/background-tasks`, `GET`/`DELETE /api/background-tasks/:id`, plus the exported `sweepOrphanedBackgroundTasks()` startup sweep) was at 22.76% -- only the two POST-error branches (limit exceeded, tmux spawn failure) had coverage before. Now at ~72% statements: validation errors, the successful-spawn 201 path, the list/by-id GET formatting (including live pane output only for a still-`running` task), DELETE's kill-vs-no-kill and no-tmux-session branches, and all three `sweepOrphanedBackgroundTasks` outcomes (no running tasks, dead session -> orphaned, alive session -> resumed polling) are now exercised. `vi.useFakeTimers()` around the spawn/sweep tests is scoped to `{ toFake: ['setTimeout', 'setInterval', ...] }` rather than faking everything -- an unscoped call also fakes `setImmediate`, which the test harness's body-delivery mechanism depends on, silently hanging the request until the suite's 15s timeout. Local baseline after this step: statements 64.69%, branches 64.03%, functions 65.41%, lines 66.06%; floor left unchanged (still ~1.4-2.4 points of buffer, comfortably covering this small an increment)
- Backend coverage -> 85% target (#751, gradual, all steps landing on this one feature branch rather than a PR per module): `src/web/routes/onboarding.ts` (the first-run onboarding wizard's write endpoints -- `POST /api/onboarding/identity`, `/claude-auth`, `/launch` -- plus the `GET /api/onboarding/status` probe; only the pure `identitySavePlan` decision core and one launch error case had coverage before) was at 18% -- now at ~93%. Local baseline after this step: statements 64.38%, branches 63.68%, functions 65.08%, lines 65.73%. Floor bumped to match (statements 62->63, branches 61->62, lines 63->64; `functions` left at 63 this step since its buffer would otherwise drop below 1.1)
...
- **[API]** `scripts/generate-error-schema.mjs` (run as part of `npm run generate:sdk`) was not idempotent: its `docs/openapi.yaml` Error-enum replacement appended an extra blank line before the following `hint:` property on every run, so a second consecutive run of `generate:sdk` on an already-regenerated spec produced a non-empty diff, requiring a manual edit to keep the PR diff clean. The replacement now reuses the newline the regex already captures ahead of `hint:` instead of inserting a second one; running `generate:sdk` twice in a row now yields an identical `docs/openapi.yaml`. The pre-existing run of accumulated blank lines left behind by the bug is also cleaned up in this change
- context guard's top-level "every tier off" early-out gate (`checkAgent` in `src/web/context-guard-runner.ts` and its pure counterpart `decideGuard` in `src/context-guard.ts`) only checked `enabled`/`saturationRestart`/`idleFlushEnabled`, not `dailyHandoffEnabled` -- an agent with ONLY the daily-handoff tier armed (the other three off) was treated as fully disarmed: its guard state was torn down every sweep and the scheduled handoff never fired. The gate now also counts `dailyHandoffEnabled`, matching the `await-handoff` phase's own (already-correct) four-way check a few lines below it
- disambiguate the audit `principal` field's source: `authPrincipal(ctx)` returns a bare name, but that namespace is not disjoint -- a human session username and a registered api-token's `name` can collide (a token named the same as a human session username would read identically to that session login in the audit trail). A new `authPrincipalSource(ctx)` in `src/web/routes/messages.ts` records which auth mechanism produced the principal (`session` / `token` / `peer` / `device` / `unknown`) as a sibling `principalSource` field in the same `detail`, alongside every existing `principal` entry, without changing `principal`'s own value or the pre-existing `authKind` field
- make three tracked hook/maintenance scripts installation-portable: `scripts/fleet-heartbeat-sweep.sh`, `scripts/hooks/auto-skillify.py`, and `scripts/hooks/skill-sql-sync.py` had the maintainer's own machine-specific absolute install path hardcoded, which both leaked into this public fork and made the scripts non-functional on any other install. Each now resolves its own install root from its file location instead (the shell script via the standard `BASH_SOURCE`-relative pattern already used elsewhere in `scripts/*.sh`; the two Python hooks via the same `__file__`-relative computation `scripts/hooks/ledger_lib.py`'s `_install_dir()` already used)
- resolve a registered api-token's name into the audit principal: a follow-up to the auth-principal fix below found that `authPrincipal(ctx)` still returned `null` for a bearer token that IS individually registered (has its own row/name in `api_tokens`), even though the auth gate could identify it -- only the legacy shared file-token genuinely lacks an identity to attribute. `resolveApiToken()` now also selects the token's `name`, threaded through `AuthResult`/`RouteContext.auth` as `tokenName` and consumed by `authPrincipal()`; a registered/named token now audits under its own name, the legacy shared token still (honestly) resolves to `null`.
- resolve the real auth principal for self-declared-from audit rows (#924): `writeAgentAuditLog` calls in `src/web/routes/messages.ts` recorded the request body's self-declared `from` as the audited `agent_id` -- any bearer-token holder (the whole fleet shares one token) could set `from` to any agent id and have the audit row read as if that agent wrote it. A new `authPrincipal(ctx)` resolves `ctx.auth?.user ?? ctx.auth?.peer ?? ctx.auth?.device ?? null` from the auth gate (never the body) and rides along in the `detail` of every affected entry (partner-sender-rejected, `pii_scrub_attempt_denied`, partner-tenant create, `pii_scrub_bypass`), so a row can tell "agent X really did this" apart from "someone used agent X's name". A plain shared-token caller resolves to `null` -- the honest answer, since the shared token alone cannot identify which agent held it for a given call.
...
- The Agents screen's org-chart (tree) view never showed the admin-only tenant-membership chip that the grid card view already had -- `GET /api/team/graph`'s nodes only carried `primaryTenantId`/`primaryTenantName` (the main-agent badge), not `tenantIds`/`tenantNames`, so `agents-org-chart.js` had nothing to render a chip from. The route now builds every non-main node's tenant fields through the same `tenantSummaryFields()` helper the grid card's `AgentSummary` already uses, and the org-chart's `renderNode()` renders the same admin-only `.tenant-chip` markup the grid card does (gated on the same `can('admin:all')`-derived admin check, now exported from `agents.js` as `isAdminView()`). Visibility matches the badge: the badge itself stays visible to every viewer, only the membership chip is admin-only, on both views alike.
