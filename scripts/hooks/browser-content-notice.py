...
-----------------------
The hook is not wired anywhere by default (not registered in
templates/settings.json.template, not in hook-registration-guard.ts's
KNOWN_HOOK_SCRIPTS -- that list only tracks scripts THIS app auto-registers
and self-heals, and this hook is never auto-registered). A fleet install
without a browser gains nothing from it and should not pay for it. An operator
who does run a browser MCP adds it to their own PostToolUse hooks in
settings.json, matcher `mcp__playwright__.*|mcp__chrome.*|WebSearch` (this
fork has no docs/security-hardening.md to point at -- this docstring is the
doc).
...
FORK ADAPTATION: dashboard visibility
--------------------------------------
Upstream only logs locally to `store/browser-content.log`. This fork's other
security hooks (see post-tool-injection-gate.py) also mirror a lightweight
record into the fleet's `hook_audit_log` table via `POST /api/hook-audit`, so
an operator sees the event on the dashboard without reading a local file. This
hook follows the same convention, but only when a pattern actually matched --
a clean page needs no dashboard row, and posting one for every single browser
read would flood the audit log with zero-signal noise (the same reasoning
post-tool-injection-gate.py applies: it only calls out on a deny, never on an
allow). The POST is best-effort and never affects the hook's own verdict --
this hook never blocks, see CONTRACT below.
...
# --- Fork adaptation: dashboard hook-audit mirror --------------------------
# Local copies of the same three resolvers post-tool-injection-gate.py uses --
# no shared "hook HTTP helpers" module exists yet in this fork, so each hook
# keeps its own (see that file for the identical pattern).
    ...
    try:
        urllib.request.urlopen(
            urllib.request.Request(
                f"http://localhost:{_web_port()}/api/hook-audit",
                data=json.dumps({
                    "agent_id": agent_id,
...
def main():
    ...
    if hits:
        ...
        content_hash = hashlib.sha256(scanned.encode("utf-8", errors="replace")).hexdigest()
        reason = ",".join(name for name, _count in hits)
        try:
            _notify_hook_audit(agent_id, payload.get("session_id"), tool_name, reason, content_hash)
        except Exception:
            pass  # notification failures never affect the hook's behavior
